...
const CountdownSettingsScreen: React.FC = () => {
  const navigation = useNavigation();
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [dataReady, setDataReady] = useState(false);
  const [state, setState] = useState<CountdownSettingsState>({interval: ''});

  const readData = useCallback(async (cancelled: () => boolean) => {
    setLoading(true);
    setDataReady(false);
    try {
      const data = await readCountdownSettings();
      if (!cancelled()) {
        setState(data);
        setDataReady(true);
      }
  ...
  const onSave = async () => {
    ...
    setSaving(true);
    try {
      await configCountdownSettings(state);
      showToast('Success');
    } catch (e) {
      ...
      <KeyboardFormScrollView style={styles.scroll}>
        ...
        <View style={styles.group}>
          <TextFieldCell
            ...
            onChangeText={t =>
              setState({interval: filterDigits(t, 2)})
            }
          />
...
export default CountdownSettingsScreen;
